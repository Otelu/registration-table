...
echipeKhion_c = Checkbutton(root, text="Echipe Kihon Dousa", variable=var12, onvalue=1, offvalue=0,
                            command=lambda: comanda())
...
# ADD Button
add_button = tk.Button(root, command=lambda: add(ws1, ws2, ws3, ws4, ws5, ws6), text="Adauga", font="Raleway")
...
def add(ws1, ws2, ws3, ws4, ws5, ws6):
        ...
        if (ani < 21) and (ani > 18):
            ...
            if var14.get() == 1:
                ws1.append([cnp, nume, club, str(int(ani)) + ' ani',
                            str(int(zi_nastere)) + '.' + str(int(luna_nastere)) + '.' + str(int(an_nastere)), 'Stagiu'])
            else:
                ws1.append([cnp, nume, club, str(int(ani)) + ' ani',
